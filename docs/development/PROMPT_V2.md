...
Каждый файл — отдельным блоком с заголовком и путём:
    ...
    Путь: `D:\Projects\IIChatTools\IIChatTools.Services\...\File.cs`
...
После всех правок — строго эти разделы:

🔨 Build + test — команды + ожидание (0 warnings, 0 errors, N/N).

🚀 Commit — here-string commit message + git add -A + push.

🧪 Smoke — что проверить после коммита.
...
Не выдумывай. Если нужен файл, которого нет:
