...
{
    /// <summary>
    /// Инструмент: выполнение JavaScript-кода через Node.js.
    /// </summary>
    public class RunJavaScriptTool : ITool
    {
        ...
        public RunJavaScriptTool(IProcessRunner processRunner, IConfiguration configuration)
        {
        ...
        /// <inheritdoc />
        public string Name => "run_javascript";

        /// <inheritdoc />
        public string Description => "Выполняет JavaScript-код через Node.js в рабочем пространстве. Возвращает stdout/stderr.";
        ...
        {
            new ToolParameterDescriptor { Name = "code", Type = "string", Description = "JavaScript-код для выполнения.", Required = true },
        ...
        public async Task<ToolResult> ExecuteAsync(ToolExecutionContext context, JObject arguments)
        {
            ...
            // Пишем временный файл внутри workspace, чтобы не загрязнять корень
            var tempDir = Path.Combine(context.WorkspaceRoot, ".tmp");
            if (!Directory.Exists(tempDir))
                Directory.CreateDirectory(tempDir);

            var tempFile = Path.Combine(tempDir, $"run_{Guid.NewGuid():N}.js");
